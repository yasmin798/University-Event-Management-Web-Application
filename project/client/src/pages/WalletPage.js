...
export default function WalletPage() {
  ...
  const fetchWallet = async () => {
    try {
      ...
      const res = await fetch("http://localhost:3001/api/wallet/balance?limit=50", {
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-gray-600 mt-2">Top up and pay for events instantly</p>
        </div>

        {/* Balance Card */}
        <div className="bg-white rounded-3xl shadow-xl p-8 mb-8 text-center transform hover:scale-105 transition-all duration-300">
          <p className="text-gray-500 text-sm uppercase tracking-wider">Current Balance</p>
          {loading ? (
            <div className="text-6xl font-bold text-emerald-600 animate-pulse">...</div>
          ) : (
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          <h2 className="text-2xl font-semibold text-gray-800 mb-4">Add Money</h2>
