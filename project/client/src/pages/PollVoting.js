...
export default function PollVoting() {
  ...
  const submitVote = async (pollId, candidateId, vendorName) => {
    ...
    try {
      ...
      showMessage(
        errorMsg.includes("already voted") ? "You already voted!" : errorMsg,
        "error"
      );
    }
  };

  const removeVote = async (pollId, candidateId) => {
    // ← now accepts candidateId
    if (!userEmail) {
    ...
    try {
      ...
      showMessage(
        err.response?.data?.error || "Failed to remove vote",
        "error"
      );
    }
  };
      ...
      >
        ...
        {polls.length === 0 ? (
          ...
          polls.map((poll) => {
            ...
            const userHasVoted =
              userEmail && poll.votedUsers.includes(userEmail);
            const userVotedFor = userHasVoted
              ? poll.candidates.find(
                  (c) =>
                    c._id.toString() ===
                    poll.votedUsers.find((e) => e === userEmail)?.candidateId
                )
              : null;
              ...
              >
                ...
                <div style={{ padding: "25px" }}>
                  {poll.candidates.map((c) => {
                    ...
                    const isLeading =
                      c.votes ===
                      Math.max(...poll.candidates.map((x) => x.votes));
                      ...
                      >
                        ...
                        <div style={{ marginTop: "12px" }}>
                          ...
                          {!userHasVoted && (
                            <button
                              onClick={() =>
                                submitVote(poll._id, c._id, c.vendorName)
                              }
                              style={{
                                width: "100%",
                                padding: "14px",
