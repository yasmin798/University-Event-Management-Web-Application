...
import {
  ...
  UserCheck,
  CheckCircle,
} from "lucide-react"; // Added CheckCircle for attendance btn
  ...
  const fetchEditRequests = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const editNotifs = notifications.filter(
          (n) => n.type === "edit_request" && n.workshopId
        );
        ...
        <div className="max-w-6xl mx-auto p-8">
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-xl p-12 text-center border border-[#c8d9e6]">
              ...
              <h3 className="text-lg font-semibold text-[#2f4156] mb-2">
                No workshops found
              </h3>
                      ...
                      <div className="flex gap-2">
                        {workshop.status === "published" && (
                          <button
                            onClick={() =>
                              navigate(
                                `/professor/workshops/participants/${workshop._id}`
                              )
                            }
                        ...
                        {isMyWorkshop &&
                          ...
                          (isPast || isOngoing) && (
                            <button
                              onClick={() =>
                                navigate(
                                  `/professor/workshops/attendance/${workshop._id}`
                                )
                              }
                              ...
                              title={
                                isPast && attendedCount === 0
                                  ? "Mark attendance & send certificates"
                                  : isPast && attendedCount > 0
                                  ? `View ${attendedCount} certificates sent`
                                  : isOngoing
                            ...
                            >
                              <CheckCircle size={16} />
                              {isPast && attendedCount > 0 && (
                                <span className="text-xs">{attendedCount}</span>
                              )}
                            </button>
                          )}
                        ...
                        {isMyWorkshop && (
                          <button
                            onClick={() =>
                              navigate(
                                `/professor/workshops/edits/${workshop._id}`
                              )
                            }
                        ...
                        {isMyWorkshop && (
                          <>
                            <button
                              onClick={() =>
                                navigate(
                                  `/professor/workshops/edit/${workshop._id}`
                                )
                              }
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                      ...
                      <div className="flex items-center gap-2 text-sm text-[#567c8d]">
                        ...
                        <span>
                          ...
                          {formatTime(workshop.endDateTime)}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-sm font-medium">
                        {" "}
                        {/* Updated: Dynamic spots with attended */}
                        <Users
                          size={16}
                          className={
                            remainingSpots <= 5
