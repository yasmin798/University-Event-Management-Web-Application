...
Application Details:
- Application ID: ${vendorTarget.raw._id || "N/A"}
- Bazaar Title: ${vendorTarget.raw?.bazaar?.title || vendorTarget.raw?.bazaarTitle || "N/A"}
- Location: ${vendorTarget.raw?.bazaar?.location || vendorTarget.raw?.location || "Not specified"}
  ...
  try {
    const response = await fetch(
      `${API_ORIGIN}/api/bazaar-applications/admin/send-qr-codes`,
      {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          boothId: qrCodeTarget.raw._id,        // keep your existing key if backend expects it
          applicationId: qrCodeTarget.raw._id,  // extra safe (won’t hurt)
