...
router.post("/", upload.array("idFiles", 5), async (req, res) => {
  try {
    const {
      ...
      platformSlot,
      startDateTime,
      vendorDescription, // ✅ NEW: from frontend form
    } = req.body;
    ...
    if (!Array.isArray(attendees) || attendees.length < 1 || attendees.length > 5)
      return res
        .status(400)
        .json({ error: "attendees must be array of 1-5" });
    ...
    let end = start
      ? new Date(
          start.getTime() + Number(durationWeeks) * 7 * 24 * 60 * 60 * 1000
        )
      : null;
    ...
    try {
      ...
      const message = `New booth vendor application pending${
        baz ? ` for bazaar '${baz.title}'` : ""
      }. Application ID: ${saved._id}`;
...
router.get("/bazaar/:bazaarId", async (req, res) => {
  try {
    ...
    const list = await BoothApplication.find({ bazaar: bazaarId })
      .populate("bazaar")
      .sort({ createdAt: -1 });
    res.json(list);
  } catch (err) {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch booth applications for bazaar" });
  }
});
...
router.post("/admin/send-qr-codes", async (req, res) => {
  ...
  try {
    ...
    if (boothApplication.status !== "accepted") {
      return res.status(400).json({
        error:
          "QR codes can only be sent for accepted booth applications",
      });
    }
    ...
    if (attendees.length === 0) {
      return res
        .status(400)
        .json({ error: "No attendees found for this booth application" });
    }
    ...
    doc.text(
      `This document contains QR codes for ${attendees.length} attendees associated with booth application ${boothId}.`,
      30,
      90
    );
